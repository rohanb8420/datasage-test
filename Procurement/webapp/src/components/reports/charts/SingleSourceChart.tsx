...
const SingleSourceChart: React.FC = () => {
  const [data, setData] = useState<{ supplier: string; count: number }[]>([]);
  useEffect(() => {
    Papa.parse('/data/supplier_performance/single_source.csv', {
      header: true,
      download: true,
      dynamicTyping: true,
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-4 rounded shadow min-h-[15rem] flex flex-col">
      <h3 className="text-lg font-semibold mb-2">Single Source Opportunities</h3>
...
export default SingleSourceChart;